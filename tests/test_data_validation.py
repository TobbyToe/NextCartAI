"""Data validation tests — self-contained inline fixtures, no external files required."""
...
def test_aisles_schema():
    assert set(_df(AISLES_CSV).columns) == {"aisle_id", "aisle"}
...
def test_departments_schema():
    assert set(_df(DEPARTMENTS_CSV).columns) == {"department_id", "department"}
...
def test_products_schema():
    assert {"product_id", "product_name", "aisle_id", "department_id"}.issubset(
        _df(PRODUCTS_CSV).columns
    )
...
def test_products_no_null_names():
    assert _df(PRODUCTS_CSV)["product_name"].notna().all()


def test_products_aisle_ids_valid():
    validate_foreign_key(_df(PRODUCTS_CSV), "aisle_id", set(_df(AISLES_CSV)["aisle_id"]))


def test_products_department_ids_valid():
    validate_foreign_key(
        _df(PRODUCTS_CSV), "department_id", set(_df(DEPARTMENTS_CSV)["department_id"])
    )
...
def test_orders_schema():
    assert {"order_id", "user_id", "eval_set", "order_number",
            "order_dow", "order_hour_of_day"}.issubset(_df(ORDERS_CSV).columns)
...
def test_orders_dow_range():
    assert _df(ORDERS_CSV)["order_dow"].between(0, 6).all(), "order_dow must be 0-6"


def test_orders_hour_range():
    assert _df(ORDERS_CSV)["order_hour_of_day"].between(0, 23).all(), "order_hour_of_day must be 0-23"


def test_orders_order_number_positive():
    assert (_df(ORDERS_CSV)["order_number"] >= 1).all()


def test_orders_eval_set_values():
    assert _df(ORDERS_CSV)["eval_set"].isin({"prior", "train", "test"}).all()
